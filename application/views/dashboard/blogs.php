...
<div style="float: right">
		<a href="<?=base_url()?>dashboard/blogs?pg=<?=base64_encode('blogs-create')?>" style="text-decoration: none">
		<button type="button" class="btn btn-success btn-circle">
			<i class="glyphicon glyphicon-plus" style="margin-left:8px"></i>           
		</button>	
		<span style="color: #ccc;"><?=$create_new_blog?></span>
		</a>
	</div>
					...
					<div class="panel-body">
						<table data-toggle="table" data-url="tables/data1.json"  data-show-refresh="true" data-show-toggle="true" data-show-columns="true" data-search="true" data-select-item-name="toolbar1" data-pagination="true" data-sort-name="name" data-sort-order="asc">
						    ...
						    <tr>
						        ...
						        <th data-field="picture"  data-sortable="true"><?=$tbl_img?></th>
...
<?php
	$cnt=1;
	$result=$this->db->query("select * from tbl_blogs ");
	if($result->result() > 0)
	{
...
?>
		<tr>
						    		<td><?=$cnt?></td>
						    		<td><?=$row->title?></td>
						    		<td><img src="<?=base_url()?>assets/dashboard/uploads/blogs/<?=$row->image_name?>" width="200" height="100" /> </td>
						    		<td>
	...
	.label2
	{
	    ...
	    color: #fff;
						    		...
						    		<td>
							    		...
							    		<a href="<?=base_url()?>dashboard/blogs?pg=<?=base64_encode('blogs-update')?>&id=<?=base64_encode($row->id)?>" style="text-decoration: none">
							    			<button type="button" class="btn btn-primary btn-circle" title="<?=$update?>">
							    				<i class="glyphicon glyphicon-check"></i>
							    			</button>
							    		</a>
							    		
							    		<a href="<?=base_url()?>dashboard/blogs?ch=del&id=<?=base64_encode($row->id)?>&img_name=<?=base64_encode($row->image_name)?>" style="text-decoration: none">	
...
?>	                            		
	                            		<a href="<?=base_url()?>dashboard/blogs?ch=down&id=<?=base64_encode($row->id)?>" style="text-decoration: none">	
...
<?php
	...
	{
?>
										<a href="<?=base_url()?>dashboard/blogs?ch=up&id=<?=base64_encode($row->id)?>" style="text-decoration: none">	
...
?>	                            		
	                            		...
	                            		<a href="<?=base_url()?>dashboard/blogs?comments=view&blog_id=<?=base64_encode(base64_encode(base64_encode(($row->id))))?>" style="text-decoration: none">	
							    			<button type="button" class="btn btn-info btn-circle" style="background: #8ad919;border-color:#8ad919 " title="<?=$mng_cmnt?>">
